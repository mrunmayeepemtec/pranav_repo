...
--3)select EMPLOYEE_ID,FIRST_NAME,LAST_NAME,SALARY FROM EMPLOYEES order by SALARY;


--4)select first_name,hire_date from employees where job_id='IT_PROG' or job_id='SA_MAN';

--5)select * from employees where employee_id in (150,160);

--6)select first_name,salary,COMMISSION_PCT,hire_date from employees where salary < 10000;

--7)select * from employees where first_name like 's%' and last_name like 's%';

--8)select * from jobs ORDER by job_title;

--9)select * from employees where commission_pct is NULL and salary between 5000and 10000;

--10)select first_name,email from employees WHERE DEPARTMENT_ID in (select DEPARTMENT_ID from departments where department_name='executive');

--11)select distinct country_id from locations;

--12)select * from employees where job_id in ('IT_PROG','FI_ACCOUNT');

--13) select * from countries order by country_name; 